...
public class EntityIceQueen extends EntityMob implements IBossDisplayData, IRangedAttackMob{
    ...
    private void summonCreature(int amount){
		...
		if(!this.worldObj.isRemote && this.summonedCreatures < 6){
		    for(int i = 0; i < amount; i++){
				...
				entity.onInitialSpawn(this.worldObj.getDifficultyForLocation(new BlockPos(d0, d1, d2)),(IEntityLivingData)null);
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase p_82196_1_, float p_82196_2_) {
		...
		double d1 = p_82196_1_.getCollisionBoundingBox().minY + (double)(p_82196_1_.height / 2.0F) - (this.posY + (double)(this.height / 2.0F));
    ...
    protected boolean teleportTo(double x, double y, double z){
		...
		if (this.worldObj.isBlockLoaded(blockPos)){
		    ...
		    while (!flag1 && blockPos.getY() > 0){
	             ...
	             Block block = this.worldObj.getBlockState(blockPosBelow).getBlock();
		    ...
		    if (flag1){
				...
				if (this.worldObj.getCollidingBoundingBoxes(this, this.getCollisionBoundingBox()).isEmpty() && !this.worldObj.isAnyLiquid(this.getCollisionBoundingBox())){
				    flag = true;
				}
		    }
		}
