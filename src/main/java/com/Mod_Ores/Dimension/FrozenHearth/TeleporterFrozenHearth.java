...
public class TeleporterFrozenHearth extends Teleporter{
    ...
    /** A private Random() function in TeleporterFrozenHearth */
    ...
    public TeleporterFrozenHearth(WorldServer par1WorldServer){
    ...
    public void placeInPortal(Entity par1Entity, float rotationYaw){
    	int dimID = this.worldServerInstance.provider.getDimensionId();
            ...
            for (int l = -2; l <= 2; ++l){
                for (int i1 = -2; i1 <= 2; ++i1){
                    for (int j1 = -1; j1 < 3; ++j1){
                        ...
                        int i2 = k + i1 * b1 - l * b0;
                        boolean flag = j1 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag ? SoulBlocks.TurquoiseBlock.get().getDefaultState() : Blocks.air.getDefaultState());
                    }
                }
            }
    ...
    public boolean placeInExistingPortal(Entity entityIn, float rotationYaw){
            ...
            for (int i1 = -128; i1 <= 128; ++i1){
                ...
                for (int j1 = -128; j1 <= 128; ++j1){
                    for (BlockPos blockpos1 = blockpos3.add(i1, this.worldServerInstance.getActualHeight() - 1 - blockpos3.getY(), j1); blockpos1.getY() >= 0; blockpos1 = blockpos2){
                        ...
                        if (this.worldServerInstance.getBlockState(blockpos1).getBlock() == SoulBlocks.TurquoiseBlock.get()){
                            while (this.worldServerInstance.getBlockState(blockpos2 = blockpos1.down()).getBlock() == SoulBlocks.TurquoiseBlock.get()){
                                blockpos1 = blockpos2;
                            }
        ...
        if (d0 >= 0.0D){
            ...
            TeleportBlockSoulForest block = (TeleportBlockSoulForest)SoulBlocks.TeleporterFrozenHearth.get();
            ...
            for (k2 = -1; k2 <= 1; ++k2){
                for (i3 = 1; i3 < 3; ++i3){
                    for (l2 = -1; l2 < 3; ++l2){
                        ...
                        i4 = j2 + (i3 - 1) * l5 - k2 * k5;
                        flag = l2 < 0;
                        this.worldServerInstance.setBlockState(new BlockPos(k3, j3, i4), SoulBlocks.TurquoiseBlock.get().getDefaultState());
                    }
                }
            }
        }
        ...
        for (k2 = 0; k2 < 4; ++k2){
            for (i3 = 0; i3 < 4; ++i3){
                for (l2 = -1; l2 < 4; ++l2){
                    ...
                    this.worldServerInstance.setBlockState(new BlockPos(k3, j3, i4), flag ? SoulBlocks.TurquoiseBlock.get().getDefaultState() : SoulBlocks.TeleporterFrozenHearth.get().getDefaultState());
                }
            }
